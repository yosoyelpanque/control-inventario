(function(root){
 ...
 function exportPack(storage){return new Promise((resolve,reject)=>{const tx=storage.db.transaction(['appData','rfidTags']);const meta=tx.objectStore('appData').get('rfidCatalog'),tags=tx.objectStore('rfidTags').getAll();tx.oncomplete=()=>resolve(meta.result?{...meta.result,records:tags.result.map(r=>{const {keys,conflict,...v}=r;return v;})}:core.empty());tx.onabort=tx.onerror=()=>reject(tx.error||Error('No se pudo leer el catálogo'));});}
