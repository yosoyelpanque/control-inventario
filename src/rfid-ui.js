(function(){
 ...
 window.addEventListener('DOMContentLoaded',()=>{
  ...
  $('rfid-export').onclick=async()=>{busy(true);try{const p=await store.exportPack(storage);if(!p.records.length)throw Error('No hay catálogo para respaldar');core.validate(p);download(JSON.stringify(p),'Catalogo_RFID_'+new Date().toISOString().slice(0,10)+'.json','application/json');message('Respaldo del catálogo generado. Puedes importarlo en otro equipo desde este panel.');}catch(e){message(e.message);}finally{busy(false);}};
  $('rfid-issues').onclick=async()=>{try{const meta=await storage.getItem('appData','rfidCatalog'),issues=meta?.issues||[];if(!issues.length){message('No hay celdas excluidas registradas.');return;}download(JSON.stringify(issues,null,2),'Revision_RFID.json','application/json');message(number(issues.length)+' celdas excluidas exportadas con archivo, hoja y fila.');}catch(e){message(e.message);}};
  ...
  async function search(conflicts=false){const id=++searchId;$('rfid-results').replaceChildren();try{
   ...
   for(const r of records.slice(0,50)){
    ...
    if(r.links.length>1){const label=el('label','Clave correcta tras revisar el bien: '),select=el('select');select.setAttribute('aria-label','Resolver '+r.tag);select.append(new Option('Mantener pendiente',''));for(const l of r.links)select.append(new Option(l.key,l.key));select.value=r.selectedKey||'';label.append(select);const confirm=el('label'),check=el('input');check.type='checkbox';confirm.append(check,document.createTextNode(' He verificado esta asociación'));const button=el('button','Guardar revisión');button.type='button';button.onclick=async()=>{if(!check.checked){message('Confirma que verificaste la asociación antes de guardarla.');return;}busy(true);try{await store.resolveTag(storage,r.tag,select.value);await refresh();await search(conflicts);}catch(e){message(e.message);}finally{busy(false);}};card.append(label,confirm,button);}
