(function(root){
 ...
 async function snapshot(storage,state,label){
  ...
  return {id:crypto.randomUUID(),at:Date.now(),label,state:structuredClone(InventoryData.clean(state)),drafts:await storage.getItem('appData','captureDrafts')||{notes:{},additional:{}},images};
 }
 ...
 async function restore(storage,id,current){
  ...
  await new Promise((resolve,reject)=>{
   const tx=storage.db.transaction(['appData','photos','layoutImages'],'readwrite');tx.oncomplete=resolve;tx.onabort=tx.onerror=()=>reject(tx.error||Error('No se pudo restaurar el punto'));
   try {
 ...
 root.InventoryRecovery={create,restore,LIMIT};
})(globalThis);
