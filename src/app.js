...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function showConfirm(title, text, onConfirm) { document.getElementById('modal-title').textContent = title; document.getElementById('modal-text').textContent = text; document.getElementById('confirmation-modal').classList.add('show'); document.getElementById('modal-confirm').onclick = () => { onConfirm(); document.getElementById('confirmation-modal').classList.remove('show'); }; document.getElementById('modal-cancel').onclick = () => document.getElementById('confirmation-modal').classList.remove('show'); }
    ...
    function recognizeAdditional(prefix) {
        ...
        if(inv||additional){
            ...
            const button=document.createElement('button');button.type='button';button.textContent='Ver '+(inv?inv['CLAVE UNICA']:additional.claveAsignada||'bien');
    ...
    function renderAreaReview(){
        ...
        for(const row of rows.slice(0,reviewLimit)){
            ...
            button.textContent='Abrir bien';button.disabled=!row.id;button.onclick=()=>row.kind==='additional'?showAdicDetail(row.id):showInvDetail(row.id);
    ...
    function reviewImport(batches, result) {
        ...
        '<p>¿Son correctos estos listados?</p><div class="review-actions"><button type="button" data-cancel>Cancelar</button><button type="button" data-confirm>Confirmar carga</button></div>';
    ...
    function renderUsers() {
        ...
        list.innerHTML = filtered.map(u => {
            const nombreArea = cleanAreaName(u.area, NOMBRES_AREAS[u.area] || 'Área Desconocida'); const isActive = state.activeResguardante?.id === u.id; const btnClass = isActive ? 'bg-green-500 text-white shadow-md' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'; const btnText = isActive ? '<i class="fa-solid fa-user-check mr-1"></i>Activo' : 'Activar';
            ...
            return `<div class="flex flex-col md:flex-row justify-between items-start md:items-center p-3 border rounded-xl bg-white shadow-sm mb-2 ${isActive ? 'border-green-500 bg-green-50 ring-2 ring-green-200' : 'hover:border-indigo-300'}"><div class="cursor-pointer mb-3 md:mb-0 w-full md:flex-1 min-w-0 pr-2" onclick="showUserDetail(${inlineValue(u.id)})"><p class="font-bold text-base text-gray-800 truncate" title="${escapeHTML(u.name)}">${escapeHTML(u.name)}</p><p class="text-sm text-gray-500 font-medium truncate"><i class="fa-solid fa-briefcase mr-1"></i>Área ${escapeHTML(u.area)} - ${escapeHTML(nombreArea)}</p><div class="mt-1 flex flex-col gap-0.5 w-full">${locTags}</div></div><div class="flex flex-shrink-0 flex-wrap gap-2 w-full md:w-auto grid grid-cols-5 md:flex items-center"><button class="w-full md:w-auto py-2.5 px-4 ${btnClass} font-bold rounded-xl text-sm" onclick="activateUser(${inlineValue(u.id)})">${btnText}</button><button type="button" class="w-full md:w-auto py-2.5 px-3 bg-indigo-100 text-indigo-700 rounded-xl text-sm" onclick="showUserDetail(${inlineValue(u.id)})" title="Ver y editar ubicaciones" aria-label="Ver y editar ubicaciones"><i class="fa-solid fa-location-dot"></i></button><button class="w-full md:w-auto py-2.5 px-3 bg-blue-100 text-blue-700 font-bold rounded-xl text-sm hover:bg-blue-200" title="Foto" onclick="showPhoto('user', ${inlineValue(u.id)})"><i class="fa-solid fa-camera"></i></button><button class="w-full md:w-auto py-2.5 px-3 bg-indigo-100 text-indigo-700 font-bold rounded-xl text-sm hover:bg-indigo-200" title="Editar" onclick="openEditUser(${inlineValue(u.id)})"><i class="fa-solid fa-pencil"></i></button><button class="w-full md:w-auto py-2.5 px-3 bg-red-100 text-red-600 font-bold rounded-xl text-sm hover:bg-red-200" title="Eliminar" onclick="deleteUser(${inlineValue(u.id)})"><i class="fa-solid fa-trash"></i></button></div></div>`
        }).join('');
    }
    ...
    window.showUserDetail = id => {
        ...
        document.getElementById('user-detail-view-location').innerHTML = (u.locations||[]).map(l => {
            ...
            return `<div class="border-b border-gray-200 pb-2 mb-2 last:border-0 last:pb-0 last:mb-0 flex justify-between items-center gap-3">
                ...
                <button type="button" class="px-3 py-2 bg-indigo-100 text-indigo-700 rounded-xl font-bold text-sm" onclick="openLocationEditor(${inlineValue(u.id)}, ${inlineValue(l)})" title="Editar ubicación">Editar</button>
                <button class="w-11 h-11 flex items-center justify-center rounded-xl transition-colors shadow-sm ${btnColor} flex-shrink-0" onclick="showPhoto('location', ${inlineValue(photoId)})" title="${hasPhoto ? 'Ver Foto de Ubicación' : 'Tomar Foto de Ubicación'}">
    ...
    function renderAdicionales() {
        ...
        document.getElementById('adicionales-list').innerHTML = list.map(i => {
            ...
            const showMatchBtn = (i.posesion === 'Cámara' || !i.posesion) && i.personal !== 'Si'; const matchBtnHtml = showMatchBtn ? `<button class="w-full md:w-auto py-2.5 px-3 bg-purple-100 text-purple-700 font-bold rounded-xl text-sm hover:bg-purple-200" onclick="openMatchModal(${inlineValue(i.id)})"><i class="fa-solid fa-link"></i></button>` : '';
    ...
    window.showInvDetail = c => {
        ...
        const invUser = state.resguardantes.find(u => u.name === i['NOMBRE DE USUARIO']); const locDetails = invUser && invUser.locationDetails && invUser.locationDetails[i.ubicacionEspecifica] ? invUser.locationDetails[i.ubicacionEspecifica] : {edificio:'N/A', piso:'N/A'}; document.getElementById('detail-view-infraestructura').innerHTML = `<i class="fa-solid fa-building mr-1"></i>${escapeHTML(locDetails.edificio)} | <i class="fa-solid fa-layer-group mr-1"></i>${escapeHTML(locDetails.piso)}`;
        if (i.UBICADO === 'SI' && i.ubicadoPor) { document.getElementById('detail-view-auditor').innerHTML = `<i class="fa-solid fa-user-check mr-1"></i> ${escapeHTML(InventoryTeam.label(i.ubicadoPor))}<br><span class="text-sm">Auxiliado por: ${escapeHTML(InventoryTeam.label(i.auxiliadoPor))}</span>`; document.getElementById('detail-view-auditor-container').classList.remove('hidden'); } else document.getElementById('detail-view-auditor-container').classList.add('hidden');
        const statusBar = document.getElementById('detail-status-indicators'); const badges = []; const isRetag = i.RE_ETIQUETADO === 'SI'; const areaError = i.UBICADO === 'SI' && invUser && i.areaOriginal !== invUser.area; if (isRetag) badges.push('<span class="detail-status-badge badge-retag"><i class="fa-solid fa-tags"></i>Reetiquetado</span>'); if (areaError) badges.push(`<span class="detail-status-badge badge-area-warn"><i class="fa-solid fa-triangle-exclamation"></i>Fuera de área (${escapeHTML(i.areaOriginal)})</span>`); if (badges.length) { statusBar.innerHTML = badges.join(''); statusBar.classList.remove('hidden'); } else { statusBar.innerHTML = ''; statusBar.classList.add('hidden'); }
    ...
    function renderNoteSuggestions() {
        ...
        for(const note of suggestions){const button=document.createElement('button');button.type='button';button.className='note-suggestion';button.textContent=note;button.onclick=()=>{input.value=note;box.replaceChildren();input.focus();};box.append(button);}
    }
    ...
    function updateNotasTabUI() {
        document.getElementById('filter-notas-active').className = window.viewArchivedNotes ? "px-4 py-2 font-bold border-b-4 border-transparent text-gray-500 hover:text-gray-700" : "px-4 py-2 font-bold border-b-4 border-indigo-500 text-indigo-700";
    ...
    document.getElementById('export-reetiquetado-btn').onclick = () => { const items = state.inventory.filter(i => i.RE_ETIQUETADO === 'SI'); if(items.length === 0) return showToast('No hay bienes para reetiquetar', 'warning'); const rows = items.map(i => ({ 'Clave Única': String(i['CLAVE UNICA']), 'Descripción': i.DESCRIPCION || i.DESCRripcion, 'Usuario': i['NOMBRE DE USUARIO'], 'Ubicación': i.ubicacionEspecifica, ...InventoryTeam.excel(i) })); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(rows), "Reetiquetado"); XLSX.writeFile(wb, `Reetiquetado_${new Date().toISOString().slice(0,10)}.xlsx`); showToast('Excel generado', 'success'); };
    ...
    document.getElementById('concil-export-btn').onclick = () => {
        ...
        const link = document.createElement("a"); link.href = URL.createObjectURL(new Blob([txt], {type: "text/plain;charset=utf-8"})); link.download = `Resumen_Conciliacion_${new Date().toISOString().slice(0,10)}.txt`; link.click();
    };
    ...
    async function renderBackupStatus() {
        ...
        const button=document.createElement('button');button.textContent=' Generar respaldo';button.className='font-bold text-indigo-900';button.onclick=()=>document.getElementById('export-session-btn').click();box.append(button);
    }
    document.getElementById('export-session-btn').onclick = async () => {
        ...
        try {
            ...
            showToast('Respaldo comprobado. Revisa que el ZIP esté en Descargas.','success');
